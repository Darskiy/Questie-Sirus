    ...
    if not journeyTreeFrame then
        ...
        journeyTreeFrame:SetCallback("OnGroupSelected", function(group)
            ...
            if e then
                ...
                local timestamp = Questie:Colorize(date( day ..', '.. month ..' %d @ %H:%M' , entry.Timestamp), Questie.COLORS.BLUE);
                ...
                if entry.Event == "Note" then
                    ...
                    note:SetText(Questie:Colorize( entry.Note , Questie.COLORS.YELLOW));
                    f:AddChild(note);
                    QuestieJourneyUtils:Spacer(f);
